...
{
    public partial class SendLog : Form
    {
        public SendLog()
        {
            InitializeComponent();
        ...
        private void SendLog_Load(object sender, EventArgs e)
        {
            con.Open();
            showPrintedMySQL();
        }
        public void showPrinted()
        {
            ...
            openRePrint.CommandText = @"SELECT * FROM printed_log";
        ...
        public void showPrintedMySQL()
        {
            ...
            string query = "SELECT no, filename, send_date, user, machine, size, modified, status FROM cpc.printed_log"; // set query to fetch data "Select * from  tabelname";
        ...
        {
            saveFileDialog1.FileName = GetTimestamp(DateTime.Now)+"_Printed.csv";
            saveFileDialog1.Filter = "Comma Delimiter (*.csv)|*.csv|All files (*.*)|*.*";

            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                String sql = "SELECT * FROM cpc.printed_log";
            ...
            //https://social.msdn.microsoft.com/Forums/en-US/6d91eac6-d990-4d64-9861-625e05c715bb/c-connect-database-and-createwrite-into-textfile?forum=csharplanguage
        }
    }
}
